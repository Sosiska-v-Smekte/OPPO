...
void FindSeasBySalinity(const std::vector<Sea>& seas, double target_salinity, double tolerance) {
    ...
    while (count < kMaxSeas && std::getline(file, line)) {
        ...
        if (pos1 != std::string::npos && pos2 != std::string::npos) {
            try {
                ...
                // Некорректные числа в строке: просто пропускаем её
                continue;
            }
...
void WriteSeasToFile(const std::string& filename, const std::vector<Sea>& seas) {
    ...
    // Записываем данные
    for (const auto& sea : seas) {
        file << sea.name << ";" << sea.depth << ";" << sea.salinity << std::endl;
    }
...
void RemoveSeaFromFile(const std::string& filename, const std::string& sea_name) {
    ...
    // Удаляем море по имени
    const auto old_size = seas.size();
    seas.erase(std::remove_if(seas.begin(), seas.end(),
        [&](const Sea& s) { return s.name == sea_name; }),
        seas.end());

    if (seas.size() == old_size) {
    ...
    WriteSeasToFile(filename, seas);
    std::cout << "Море успешно удалено из файла. Осталось " << seas.size() << " морей." << std::endl;
}
